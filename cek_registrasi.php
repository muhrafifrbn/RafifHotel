...
if(isset($_POST['registrasi'])){
    if(!empty($_POST['username'] || !empty($_POST['password']) )){
        ...
        if(mysqli_affected_rows($koneksi) > 0){
            // $ambilId = mysqli_query($koneksi, "SELECT * FROM akun WHERE username = '$username'");
            // $id = mysqli_fetch_assoc($ambilId)['id'];
            // $tambahHakAkses = mysqli_query($koneksi,  "INSERT INTO akses VALUES($id,'$pengguna')");
            echo "<script>
